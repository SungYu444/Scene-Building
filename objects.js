function setAttribPointers(obj) {
    ...
    if(obj.tBuffer && obj.vTexCoord !== undefined && obj.vTexCoord >= 0) {
...
function setBuffers(obj, program)
{
    ...
    if(obj.texCoordArray && obj.texCoordArray.length > 0) {
        obj.tBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, obj.tBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, flatten(obj.texCoordArray), gl.STATIC_DRAW);
...
SphereSub.triangle = function (a, b, c) {
    ...
    var nc = vec3(c[0],c[1],c[2]) ;
     ...
     this.texCoordArray.push(vec2(0.0,0.0));
     this.texCoordArray.push(vec2(0.0,0.0));
     this.texCoordArray.push(vec2(0.0,0.0));
...
function AddInAttribArrays(obj, v)
{
    ...
    if(!obj.texCoordArray) {
        obj.texCoordArray = [];
    }
    obj.texCoordArray.push(v.texCoord);
}
